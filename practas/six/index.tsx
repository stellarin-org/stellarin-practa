...
import { useAICoPlayer, AICoPlayerBubble } from "./AICoPlayer";
...
export default function MyPracta({ context, onComplete, showSettings, onSettings }: {
  ...
  const liveKeyboardStates = useMemo(() => {
    ...
    if (currentGuess.length > 0) {
      const states = evaluatePartialGuess(currentGuess, targetWord);
      for (let i = 0; i < currentGuess.length; i++) {
        const letter = currentGuess[i];
        const state = states[i];
        if (state === "pending") continue;
        const existing = live[letter];
        if (!existing ||
            state === "correctDouble" ||
            (state === "correct" && existing !== "correctDouble") ||
            (state === "present" && !existing) ||
            (state === "absent" && !existing)) {
          live[letter] = state;
        }
      }
    }
  ...
  const updateKeyboardStates = useCallback((guess: string, states: LetterState[]) => {
    setLetterStates((prev) => {
      ...
      for (let i = 0; i < guess.length; i++) {
        const letter = guess[i];
        const state = states[i];
        const existing = newStates[letter];
        if (!existing ||
            state === "correctDouble" ||
            (state === "correct" && existing !== "correctDouble") ||
            (state === "present" && existing === "absent")) {
                ...
                >
                  <View style={styles.resultTextGroup}>
                    ...
                    <ThemedText style={[styles.resultSubtitle, { color: theme.textSecondary }]}>
                      {gameState === "won"
                        ? "You got the word for the day, come back tomorrow!"
                        : "The word for the day — come back tomorrow!"}
                    </ThemedText>
                  </View>
